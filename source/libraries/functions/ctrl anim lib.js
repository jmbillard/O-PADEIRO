...
function lockTrmProp(sLayer) {
	...
	for (var p = 1; p <= trm.numProperties; p++) {
		...
		if (!prop.canSetExpression || prop.numKeys > 0 || prop.expression != '') continue;
...
function setHierarchy(sLayer, cLayer) {
	...
	if (sLayer.threeDLayer || sLayer instanceof LightLayer) cLayer.threeDLayer = true;
	...
	for (var i = 1; i <= sTrm.numProperties; i++) {
		...
		if (sTrm.property(i).dimensionsSeparated) cTrm.property(i).dimensionsSeparated = true;
	}
}
...
function findCenter(lArray) {
	...
	for (var i = 0; i < lArray.length; i++) {
		var lPos = lArray[i].property('ADBE Transform Group').property('ADBE Position'); // Obtém a propriedade de posição da camada
